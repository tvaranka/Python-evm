...
def load_video(video_path):
    ...
    assert os.path.exists(
        video_path), "File/folder not found, make sure it exists and is in the current directory."
...
def _magnify(video, alpha, lambda_c, r1, r2, n_levels):
    ...
    for i in range(1, F):
        ...
        lowpass2 = [r2 * pyr[k] + (1 - r2) * lowpass2[k]
                    for k in range(n_levels + 1)]
...
def _preprocess(video):
    assert isinstance(video, (np.ndarray, torch.Tensor)
                      ), "The video should be either a numpy array or a torch tensor"
    assert len(video.shape) == 3 or len(video.shape) == 4, "The shape of the video should be either (F, H, W) or \
                                                            (F, H, W, C)"
    F, H, W, C = _shape(video)
    assert C == 3 or C == 1, "The shape of the video should be either (F, H, W) or \
                                                            (F, H, W, C)"
...
def _save_directory(video, folder_path):
    ...
    F, H, W, C = _shape(video)
    for i in range(F):
        save_name = folder_path + "/" + \
            folder_path.split("/")[-1] + str(i) + ".jpg"
